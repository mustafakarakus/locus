...
The source installer automatically initializes the Git project it is run from.
`locus init` remains available as an idempotent repair/setup command for projects
cloned later or installations performed by a package manager.
...
One honest caveat: a package manager such as Homebrew or Cargo installs binaries
globally and is not told which project should be modified. It therefore cannot
safely inject per-project rules, MCP files, or Git hooks into an arbitrary
repository. The source installer can configure its invocation project; projects
cloned later are initialized once with `locus init`. Agent-side saving still
depends on the host following the injected protocol unless that host exposes a
